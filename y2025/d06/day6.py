...
with open("input.txt") as f:
    ...
    inputs = [parse(line) for line in lines[:-1]]
    ops = lines[-1].split()
...
transposed_lines = list("".join(digits) for digits in zip_longest(*lines, fillvalue=" "))
raw_eqns:list[list[str]] = list(split_on_blank(transposed_lines))
eqns = [parse_eqn2(eqn) for eqn in raw_eqns]
